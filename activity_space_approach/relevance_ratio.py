import pandas as pd
from tqdm.auto import tqdm  # For the progress_apply function
def calculate_day_night_duration(df):
    # Pre-calculate all datetime boundaries at once for vectorized operations
    df = df.copy()
    df['start_date'] = df['start_time'].dt.date
    df['end_date'] = df['end_time'].dt.date

    # Create date boundaries for the entire dataframe at once
    ...
    def process_time_chunks(row):
        if not row['spans_multiple_days']:
            # Single day logic - more straightforward calculation
            ...
            day_mins = max(0, full_days * 15)  # 15 hours of daytime per full day
            night_mins = max(0, full_days * 9)  # 9 hours of nighttime per full day

            # Handle start date partial day
            if row['starts_before_morning']:
                night_mins += (row['morning_bound_start'] - row['start_time']).total_seconds() / 3600
                day_mins += 15  # Full day portion
                night_mins += 2  # Evening portion
            elif row['starts_after_evening']:
    ...
    # Process in chunks using pandas groupby for better memory management
...
def calculate_night_relevance_pre_disaster(df_pre_earthquake_stays, df_post_earthquake_stays):
    ...
    # Aggregate durations for the same locations (clusters)
